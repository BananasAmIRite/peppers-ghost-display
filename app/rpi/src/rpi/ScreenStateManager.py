...
class ScreenStateManager(HostManager):
        ...
        if type == comms.PI_MSG and len >= 1:
            ...
            if datatype == comms.PI_SWIPE:
                ...
                if swipe_dir == 0:
                    ...
                    self.swipe_down()
            elif datatype == comms.PI_ADD_TMP_SCRN:
                scrn = data[1]
                self.add_temp_screen(ScreenType(scrn))
            elif datatype == comms.PI_ADD_TMP_NTF:
                scrn = data[1]
                self.add_temp_notif(ScreenType(scrn))
            elif datatype == comms.PI_RMV_TMP_SCRN:
                scrn = data[1]
                self.remove_temp_screen(ScreenType(scrn))
