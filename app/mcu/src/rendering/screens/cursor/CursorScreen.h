...
class CursorScreen : public SpriteScreen {
    ...
    public: 
        CursorScreen(Adafruit_ImageReader& reader) : SpriteScreen(reader, std::vector<std::string>{
            "/cursor.bmp"
        }, true) {
        ...
        void render(Adafruit_GFX* tft) override {
            ...
            Serial.print("Drawing cursor: ");
            Serial.print(state->cursorX); 
            Serial.print(", "); 
            Serial.println(state->cursorY); 
